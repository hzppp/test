					...
					<view class="pic-text" @tap="toActivityPage(item)">
						...
						<view class="desc">
							<view class="left-area">
								<view>
									<view class="locationCot" v-if="item.type==3">
										<view class="cityName">{{ selectCity || indexCity.name }}</view>
										<view class="line"></view>
									</view>
									<view class="date">{{item.startTime | dateFilter}} - {{item.endTime | dateFilter}}
									</view>
								</view>
								<view class="info">
									<view v-show="item.visitCount"><span
											class="num">{{item.visitCount | numFilter}}</span>人感兴趣</view>
									<view v-show="item.visitCount && item.clueCount " class="line"></view>
									<view v-show="item.clueCount "><span
											class="num">{{item.clueCount | numFilter}}</span>人报名</view>
								</view>
							</view>
							<view class="right-area">
								<view class="tag before"
									v-show="statusFilter(item.startTime, item.endTime) == 'before'">即将开始</view>
								<view class="tag during"
									v-show="statusFilter(item.startTime, item.endTime) == 'during'">进行中</view>
								<view class="tag after" v-show="statusFilter(item.startTime, item.endTime) == 'after'">
									已结束</view>
							</view>
						</view>
					</view>
				...
				try {
					...
					if (currentLocation) {
						...
						const crtLocationProvinceItem = this.provinceList.find(item => item.name.replace('省', '')
							.replace('市', '') == currentLocation.selectedCityData.pro.replace('省', '').replace('市',
								''))
						if (crtLocationProvinceItem) {
