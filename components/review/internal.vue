<template>
	<view class="internal" v-if="internalData.name">
		<view class="internal__content">
			<view class="internal__content__title"><i class="icon" />{{ internalData.name }}</view>
			<view class="classify">
				<view class="classify__item" v-for="(item, index) in internalData.parData" :key="index">
					<uni-collapse @change="change">
						...
						<uni-collapse-item class="diy_uni-collapse-item" :title="item.value" showAnimation="true" :count="item.amount">
							<view class="item__model_wrap">
								...
								<view class="view--more" v-if="item.amount  > 1" @tap='goCanpei(internalData.parData[index].modelIds)'>查看{{ item.amount }}款车型完整参数<i class="right_icon" /></view>
							</view>
						</uni-collapse-item>
					</uni-collapse>
				</view>
	...
	.internal__content {
		...
		.internal__content__title {
			...
			.icon {
				...
				height: 30rpx;
				margin-right: 18rpx;
				background-image: url("../../static/images/internal_icon.png");
